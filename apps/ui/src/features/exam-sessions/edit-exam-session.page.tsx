...
import { fetchExamSessionById, updateExamSession } from './exam-sessions.api';

/**
 * Form di modifica di una sessione d'esame (solo SEGRETERIA).
 * Come la creazione, ma precompila i campi leggendo la sessione dall'id in URL.
 */
export function EditExamSessionPage() {
  ...
  // Precompila il form leggendo la sessione corrente.
  useEffect(() => {
    fetchExamSessionById(sessionId)
      .then((session) => {
        setName(session.name);
        setStartDate(session.startDate);
        setEndDate(session.endDate);
        setPlanningStartDate(session.planningStartDate);
        setPlanningEndDate(session.planningEndDate);
      })
      .catch((err) =>
        setError(err instanceof Error ? err.message : 'Sessione non trovata')
      )
