...
import { fetchSubjects, fetchMySubjects } from '../subjects/subjects.api';
...
/**
 * Form di modifica di un esame.
 * - SEGRETERIA: può modificare qualsiasi esame e assegnare qualsiasi materia.
 * - DOCENTE: può modificare solo i propri esami e solo verso proprie materie.
 * Le regole di dominio restano verificate dal backend.
 */
export function EditExamPage() {
  ...
  // Opzioni dei <select>: materie secondo il ruolo, sessioni sempre tutte.
  useEffect(() => {
    const loadSubjects = isSegreteria ? fetchSubjects : fetchMySubjects;
    loadSubjects()
      .then(setSubjects)
      .catch(() => setSubjects([]));
    fetchExamSessions()
  ...
  // Precompila il form leggendo l'esame corrente.
  useEffect(() => {
    fetchExamById(examId)
      .then((exam) => {
        ...
        setDate(exam.date);
        setStartHour(exam.startHour);
        setEndHour(exam.endHour);
        setType(exam.type);
        setRoomType(exam.roomType);
      })
      .catch((err) =>
        setError(err instanceof Error ? err.message : 'Esame non trovato')
      )
  ...
  return (
    <div className="mx-auto max-w-lg">
      <h1 className="mb-4 text-2xl font-bold text-slate-800">Modifica esame</h1>
